...
import de.lclutz.simulator.prozessor.Pic16F18aAdapter;
...
import de.lclutz.simulator.prozessor.util.Programme;
...
public class Simulator implements Runnable {
	...
	Prozessor prozessor = Pic16F18aAdapter.create();
	...
	private void initialize() {
		frame = new JFrame();
		frame.setBounds( 100, 100, 894, 518 );
		...
		schritt.setBounds( 779, 11, 89, 23 );
		...
		ladenButton.setBounds( 779, 227, 89, 23 );
		...
		btnProgramm.setBounds( 292, 114, 29, 23 );
		frame.getContentPane().add( btnProgramm );

		JScrollPane scrollPane = new JScrollPane();
		scrollPane.setBounds( 330, 11, 439, 457 );
	...
	void ladeProzessor(String programmText) {
		...
		ladeProzessorMitProgrammzeilen( Arrays.asList( programmText.split( "\n" ) ) );
	}

	void ladeProzessorMitProgrammzeilen(List<String> programmText) {
		List<Long> programm = new ArrayList<>();
		// entferne kommentar/leerzeichen und fuelle programm
		programmText.stream()
			.map( l -> entferneKommentar( l ) )
			.filter( s -> s.length() > 0 )
			.map( l -> leseZweiteZahl( l ) )
			.filter( s -> s.length() > 0 )
				.forEach( l -> programm.add( Long.parseLong( l, 2 ) ) );
	...
	static String leseZweiteZahl(String zeile) {
